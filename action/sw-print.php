...
if (!isset($_COOKIE['COOKIES_MEMBER']) or !isset($_COOKIE['COOKIES_COOKIES'])) {
  ...
  switch (@$_GET['action']) {
      ...
      /* -------  CETAK PDF----------*/
    case 'pdf':
      $query = "SELECT * FROM karyawan WHERE  npp='$row_user[npp]'";
...
<body>';
    ...
    <section class="container_box">
      <div class="row">';
        if (isset($_GET['from']) or isset($_GET['to'])) {
          echo '<h3>DATA ABSENSI  PER TANGGAl ' . tanggal_ind($from) . ' S/D ' . tanggal_ind($to) . '</h3>';
        } else {
          echo '<h3>DATA ABSENSI  BULAN ' . tanggal_indo($month_en) . ' ' . $year . '</h3>';
        }
        echo '
